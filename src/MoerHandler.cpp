...
MoerHandler::MoerHandler() : moerState(MOER_STATE::NONE), renderProgress(0) {
   if (std::filesystem::exists(moerSettingsFileName)) {
      ...
      moerPath = moerSettings[moerPathSettingName];
   } else {
      moerSettings[moerPathSettingName] = "";
      moerSettings["fileDescription"] =
          "This file is generated by MoerScenePreviewer";
   }
}

void MoerHandler::setMoerPath(const std::string& moerPath_) {
   moerPath = moerPath_;
}

void MoerHandler::writeMoerPath() {
   moerSettings[moerPathSettingName] = moerPath;
   std::ofstream o(moerSettingsFileName);
   o << moerSettings;
}

void MoerHandler::setMoerPathAndWrite(const std::string& moerPath_) {
   setMoerPath(moerPath_);
   writeMoerPath();
}

std::string& MoerHandler::getMoerPath() { return moerPath; }
...
int MoerHandler::executeMoer(const std::filesystem::path& moerPath,
   ...
   if (!moerThreadRunning) {
      renderProgress.store(0);
      moerOutputThread =
          std::thread(&MoerHandler::handleMoerStdOutput, this, &moerProcess);
      moerOutputThread.detach();
      moerThreadRunning = true;
   }
